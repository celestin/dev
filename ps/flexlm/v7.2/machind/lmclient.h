/*****************************************************************************r
	    ...
	    COPYRIGHT (c) 1988, 2001 by Globetrotter Software Inc.
...
#define FLEXLM_VERSION 7
#define FLEXLM_REVISION 2
#define FLEXLM_PATCH "d"
#define FLEXLM_COPYRIGHT_YEAR "2001"
...
#define LM_BORROW_NOCOUNTER -103 /* "No counters available in borrow meter" */
#define LM_BORROWED_ALREADY -104 /* "Feature already borrowed to local meter" */
...
#define LM_BEHAVIOR_CURRENT	"07.1"
...
#define LM_CODE_GEN_INIT(code) \
	...
	memcpy(&(code)->pubkeysize, lm_prisize, sizeof(lm_prisize)); \
	(code)->pubkey_fptr = l_prikey_sign; \
	{ \
	  int i; \
		for (i = 0; i < LM_PUBKEYS; i++) \
		{ \
			memcpy((code)->pubkey[i], lm_prikey[i], \
							lm_prisize[i]); \
		} \
	}
...
#if defined (PC) && !defined(FLEXLM_DLL) /* unavailable on Unix */
...
#define LM_CONF_DECIMAL_FMT 	LM_CONF_DECIMAL_FMT_V6
			...
			int strength_override;	/* lic gen only */
 			char hash[30]; 	/*- put hash here for
