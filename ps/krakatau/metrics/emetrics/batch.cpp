...
void createHTMLIndexes(OurSQL *conn, const char *dir, int diff_length, const char *cat_str) {
	...
	for (int i=nLevels,l=0 ; i>0 ; i--,l++) {
		...
		itoa(l,ln) ;
/*		strcpy(index_filename,conn->cell(k*inc,1).c_str()) ;
		
		if ((k*inc+inc-1)<conn->rows()){
			strcat(index_filename,conn->cell(k*inc+inc-1,2).c_str()) ;
		} else {
			strcat(index_filename,conn->cell(conn->rows()-1,2).c_str()) ;
		}

		index.clear() ;
		index.open(index_filename) ;

		index << "just testing" << endl ;*/
...
//	index << "\t\t\tHREF=\"0_class_0.html\" target=\"nav\">Classes</a><BR><A\n" ;
//	index << "\t\t\tHREF=\"0_method_0.html\" target=\"nav\">Methods</a><BR>\n" ;
	index << "\t\t</TR>\n\n" ;
...
//					metnames[metindex] = conn->cell(k,10) ;
...
// MULTIPLE HTML STUFF
			if (conn->rows()>0) {
				...
				//strcat(htmlqry,conn->cell(conn->rows()-1,1).c_str()) ;
				//strcat(htmlqry,"','") ;
				//strcat(htmlqry,jp) ;
				//strcat(htmlqry,".html')") ;
...
//					old_catid[index] = conn->intCell(j,index)  ;
					index -- ;
				}
...
void createPMTemporaryTables(OurSQL *conn, int status) {
	// TODO optimize - some of these queries are the same each 
	// time
