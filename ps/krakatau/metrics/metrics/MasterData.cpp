...
/*const int MasterData::COMMON_MTG = 1 ;
const int MasterData::PROJECT_MTG = 2 ;
const int MasterData::FILE_MTG = 3 ;
const int MasterData::CLASS_MTG = 4 ;
const int MasterData::INTERFACE_MTG = 5 ;
const int MasterData::METHOD_MTG = 6 ;*/
...
/*MasterData::MasterData(string dbName): theBuffer(100),
	theTempBuffer(1000,true), theDBConnection(dbName),
	theCurrentTypeID(USERTYPES)
{
	theDBConnection.connect() ;
}
...
MasterData::~MasterData()
{
	theLog.close() ;
	theBuffer.commit() ;
	theDBConnection.disconnect() ;
}*/


/*long MasterData::codeMtt(long metricID, long groupID)
{
	return ((groupID * METRIC_FACTOR) + metricID) ;
}
...
/*char *dtoa(double d, char *buff, int buffsize)
{
	char tmp[20] ;
	int intval = (int) d ;
	float frac = fabs( (float) (d - intval) ) ;

	if ( frac < 1e-5 )
		return ltostr(intval,buff,buffsize) ;

	strcpy(buff,ltostr(intval,buff,buffsize)) ;
	strcat(buff,".") ;
	...
	{
		frac *= 10 ;
		strcat(buff,ltostr((int)frac,tmp,10)) ;
		frac -= (int) frac ;
	}
...
char *ltostr(long l, char *buff, int buffsize)
{
	/*char *retval = buff + buffsize ;
	*retval = '\0' ;

	retval = lltostr(abs(l),retval) ;
...
/*long MasterData::getMetricGroup(int catID)
{
	if (catID == MasterData::PROJECT_CAT)
		return MasterData::PROJECT_CAT ;

	else if (catID == MasterData::FILE_CAT)
		return MasterData::FILE_CAT ;

	else if (catID == MasterData::CLASS_CAT)
		return MasterData::CLASS_CAT ;

	else if (catID == MasterData::INTERFACE_CAT)
		return MasterData::INTERFACE_CAT ;

	else if (catID == MasterData::METHOD_CAT)
		return MasterData::METHOD_CAT ;
