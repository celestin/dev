...
//	theTLOC = 0 ;
	...
	float SEIMI = 0 ;
	float aveLOC = 0 ;	// avg lines of code
	float perCM = 0 ;	// avg percentage of lines of comments per module
	float NMETH = theProjNode.getMetric(MasterData::NMETH_MET) ;
	float sumV = theProjNode.getMetric(MasterData::SUMV_MET) ;
	float sumVg = theProjNode.getMetric(MasterData::SUMVG_MET) ;
	...
	if ((NMETH > 0) && (sumV > 0) && (sumVg > 0))
	{
		...
		SEIMI = 171 - 5.2 * log(sumV) - 0.23 * sumV -
			16.2 * log(aveLOC) - 50 * sin(sqrt(2.4 * perCM)) ;
	}

	theProjNode.setMetric(MasterData::SEIMI_MET, SEIMI) ;
}
...
void MetricPhase::calc_CBO(SymbolNode &node)
{
	...
	types = node.getUniqueTypesOfChildren(MasterData::FIELD_CAT, types) ;
	...
	for( i = 0 ; i < node.childCount() ; i++ )
	{
		...
		{
			types = child.getUniqueTypesOfChildren(MasterData::FIELD_CAT, types) ;
		}
	}
...
void MetricPhase::calc_LCOM(SymbolNode &node)
{
	...
	char sql[512], charNumber[30] ;
		...
		/*strcpy(sql,"insert lcom select s2.symid from symbol s1 inner join link lk on s1.symID = lk.symID inner join symbol s2 on s2.symID = lk.sym2ID where s1.symParentID=") ;
...
//					MasterData::theLog << "maxflds: " << maxflds << endl ;

					usage[thisrow] = new long[maxflds+1] ;
					usage[thisrow][0] = 0 ;
				}/* else {
					MasterData::theLog << "b" << endl ;
					usage[thisrow] = NULL ;
				}*/
	...
	// OLD CODE - CAM translated from GNS
	// This old code does not calculate C&K LCOM, but something which
	// GNS invented (albiet a useful guage of attribute usage) - 'LOCM'
	/*if ( (CSO > 0) && (CSA > 0) )
	{
		countMethods = node.getAttributeUsage(countMethods) ;

		usage = node.getAttributeUsage() ;

		int i=0 ;	// DEBUG
		MasterData::theLog << "Node: " << node.getName() << endl ;	// DEBUG
		MasterData::theLog << "CSA: " << CSA << endl ;	// DEBUG
		MasterData::theLog << "CSO: " << CSO << endl ;	// DEBUG

		for(iter = countMethods.begin(); iter < countMethods.end(); iter++)
		{
			MasterData::theLog << ++i << ": " << (float) 100 * (float) ((CSO - (float) (*iter)) / CSO) << endl ;	// DEBUG
			LOCM += (float) 100 * (float) ((CSO - (float) (*iter)) / CSO) ;
		}

		MasterData::theLog << "LOCM: " << LOCM << endl ; // DEBUG
		LOCM /= CSA ;
	}*/
}
