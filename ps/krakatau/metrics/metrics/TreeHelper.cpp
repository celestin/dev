...
void TreeHelper::writeSymbol(long symID, long parID, const char *symName, long sytID, long catID, long prtID)
{
	...
	if (theNSymWrite>=MAX_QUERY_LENGTH)
	{
		flushWriteSymbol() ;
...
void TreeHelper::buildAncestry()
{
	...
	createTable2 = strdup(" (symid int not null, catid int not null, symname varchar(128), psymid int not null, pcatid int not null, psymname varchar(128) not null, filid int, filname varchar(128), primary key (symid), unique index (symid))") ;

	insertTable2 = strdup(" select s1.symID as symID, s1.catID as catID, s1.symName as symName, s2.symID as psymID, s2.catID as pcatID, s2.symName as psymName, s2.symID as filID, s2.symName as filName from symbol s1 inner join ") ;
	insertTable3 = strdup(" s2 on s1.symparentid=s2.symid where s2.catid=4") ;

	insertTable4 = strdup(" select s1.symID as symID, s1.catID as catID, s1.symName as symName, s2.symID as psymID, s2.catID as pcatID, s2.symName as psymName, s2.filID as filID, s2.filName as filName from symbol s1 inner join ") ;
	insertTable5 = strdup(" s2 on s1.symparentid=s2.symid where s1.catid<>8") ;
	...
	createTable2 = strdup(" (symid int not null, symname varchar(128), psymid int not null, psymname varchar(128) not null, subdepth int not null, primary key (symid), unique index (symid))") ;
	insertTable2 = strdup(" select symid, symname, symparentid as psymid, concat('/',symname) as psymname, 1 from symbol where symparentid=1 and catid=1") ;
	insertTable4 = strdup(" select s1.symid, s1.symname, s1.symparentid, concat(sp.psymname,'/',s1.symname), ") ;
	insertTable5 = strdup(" from symbol s1 inner join ") ;
	insertTable6 = strdup(" sp on s1.symparentid=sp.symid where catid=1") ;
