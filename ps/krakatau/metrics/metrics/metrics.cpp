...
void buildProjNames() {
  ...
  for(int i=0; i<nRoots; i++) {
    ...
    strcpy_s(sql,1024, "select prjName from project where prjID='") ;
    strcat_s(sql,1024, projNames[i].c_str()) ;
    strcat_s(sql,1024, "'") ;
...
char *buildProgressText(int prevPhasesTotal, int currPhaseTotal, char *prog, bool showName)
{
  ...
  strcpy_s(progressText, 512, ltostr(((long)stageProgress),buff,BUFFSIZE)) ;
  strcat_s(progressText, 512, ",") ;
  strcat_s(progressText, 512, ltostr((long)overallProgress,buff,BUFFSIZE)) ;
  strcat_s(progressText, 512, ",") ;
  strcat_s(progressText, 512, prog) ;
#if defined(PM)
  if (showName) {
    strcat(progressText, " (") ;
