...
# Check for products that don't support Buy Online
#if ($FORM{'kbwincpp'} eq 'y')    { &product_msg('kb') ; }
#if ($FORM{'kbwinjava'} eq 'y')   { &product_msg('kb') ; }
#if ($FORM{'kbsolcpp'} eq 'y')    { &product_msg('kb') ; }
#if ($FORM{'kbsoljava'} eq 'y')   { &product_msg('kb') ; }
#if ($FORM{'pmwincppv2'} eq 'y')  { &product_msg('pm') ; }
#if ($FORM{'pmwinjavav2'} eq 'y') { &product_msg('pm') ; }
#if ($FORM{'pmsolcppv2'} eq 'y')  { &product_msg('pm') ; }
#if ($FORM{'pmsoljavav2'} eq 'y') { &product_msg('pm') ; }
...
$actpage.="<FORM name=\"details\" action=\"/cgi-bin/buy.pl\" method=post>" ;

$actpage.="<table border=0 cellspacing=0 cellpadding=4>\n" ;
$actpage.="<tr><td colspan=2 class=\"fldbad\">".$warning_text_insertion.&selected_products().&country_fields()."</td></tr>\n" ;
...
sub selected_products { 
  ...
  if ($FORM{'kp'} eq 'y') { $selprod.=&input_field('hidden', 'kp', 1) ; }
  if ($FORM{'pm'} eq 'y') { $selprod.=&input_field('hidden', 'pm', 1) ; }
  if ($FORM{'kl'} eq 'y') { $selprod.=&input_field('hidden', 'kl', 1) ; }
  if ($FORM{'em'} eq 'y') { $selprod.=&input_field('hidden', 'em', 1) ; }
  if ($FORM{'vz'} eq 'y') { $selprod.=&input_field('hidden', 'vz', 1) ; }
...
#  if ($FORM{'vzwinjava'} eq 'y') { $selprod.=&input_field('hidden', 'vzwinjava', 1) ; }
#  if ($FORM{'vzsoljava'} eq 'y') { $selprod.=&input_field('hidden', 'vzsoljava', 1) ; }
...
#  
#  if ($prodcode eq 'pm') {
#    $actpage="<h1 class=\"narr\">Krakatau Project Manager</h1>\n" ;
#    $actpage.="<p class=\"narr\">Please contact <a href=\"mailto:sales@powersoftware.com\">"
#      . "sales@powersoftware.com</a> regarding the purchase of Krakatau Project Manager.</p>\n" ;
#  } elsif ($prodcode eq 'kb') {
#    $actpage="<h1 class=\"narr\">Krakatau kbatch</h1>\n" ;
#    $actpage.="<p class=\"narr\">Krakatau kbatch will be available in July 2001.  Please "
#      . "contact <a href=\"mailto:sales@powersoftware.com\">"
#      . "sales@powersoftware.com</a> for further information or alternatively, you may "
#      . "wish to download <a href=\"/kl/\">Krakatau Lite.</a></p>\n" ;
#  }
#    
#  $page=&get_file_contents($newfile) ;
#  $page=~s/$separator\s*/$actpage/ ;
