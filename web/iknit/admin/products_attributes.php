 ...
 *
 * Who  When         Why
 * CAM  28-Apr-2010  10636 : Added Static Options
 * CAM  05-Jun-2010  10684 : Added Product Option "top category".
 * * * * * * * * * * * * * * * * * * * * * * * */
  ...
  if (tep_not_null($action)) {
    switch ($action) {
      case 'add_product_options':
        ...
        $option_category_array = $HTTP_POST_VARS['option_category'];
        $option_static = $HTTP_POST_VARS['option_static'] == 'on' ? '1' : '0';
        $option_visible = $HTTP_POST_VARS['option_visible'] == 'on' ? '1' : '0';
        ...
        for ($i=0, $n=sizeof($languages); $i<$n; $i ++) {
          ...
          $sql =
            "insert into " . TABLE_PRODUCTS_OPTIONS . " (".
              ...
              "products_options_is_static, products_options_is_visible,categories_id".
            ") values ('" .
              (int)$products_options_id . "', '" . tep_db_input($option_name) . "', '" . (int)$languages[$i]['id'] . "', ".
              "'". $option_static . "', '" . $option_visible . "', '" . (int)$option_category_array[$languages[$i]['id']] . "')";
          tep_db_query($sql);
        }
      ...
      case 'update_option_name':
        ...
        for ($i=0, $n=sizeof($languages); $i<$n; $i ++) {
          ...
          tep_db_query("update " . TABLE_PRODUCTS_OPTIONS . " set products_options_name = '" . tep_db_input($option_name) .
           "', products_options_is_static = '" . $option_static .
           "', products_options_is_visible = '" . $option_visible .
           "', categories_id = '" . (int)$option_category_array[$languages[$i]['id']] .
           "' where products_options_id = '" . (int)$option_id . "' and language_id = '" . (int)$languages[$i]['id'] . "'");
        }
...
// For our static properties
static_options = new Array();
len = 0;
...
function toggleT(selected) {
 for(i=0;i<static_options.length;i++){
   if(selected == static_options[i]){
     if (document.all) { // is IE
       document.getElementById("svalue_price").style.visibility = "hidden";
       document.getElementById("sprice_prefix").style.visibility = "hidden";
       return;
     }
   }
 }
 if (document.all) { // is IE
     document.getElementById("svalue_price").style.visibility = "visible";
     document.getElementById("sprice_prefix").style.visibility = "visible";
 }
}
//--></script>
</head>
...
<!-- options and values//-->
      <tr>
        <td width="100%"><table width="100%" border="0" cellspacing="0" cellpadding="0">
          <tr>
            <td valign="top" width="50%"><table width="90%" border="0" cellspacing="0" cellpadding="2">
<!-- options //-->
<?php
...
?>
              <tr>
                <td colspan="6" class="pageHeading">&nbsp;<?php echo HEADING_TITLE_OPT; ?>&nbsp;</td>
              </tr>
              <tr>
                <td colspan="6" class="smallText" align="right">
<?php
    $options =
      "select * from " . TABLE_PRODUCTS_OPTIONS . " po INNER JOIN ". TABLE_CATEGORIES_DESCRIPTION . " cd ".
      "ON cd.categories_id = po.categories_id ".
      "AND cd.language_id = po.language_id ".
      "WHERE po.language_id = '" . (int)$languages_id . "' ".
...
?>
              ...
              <tr class="dataTableHeadingRow">
                ...
                <td align="center" class="dataTableHeadingContent"><?php echo TABLE_HEADING_OPT_STATIC; ?></td>
                <td align="center" class="dataTableHeadingContent"><?php echo TABLE_HEADING_OPT_VISIBLE; ?></td>
                <td class="dataTableHeadingContent">&nbsp;<?php echo TABLE_HEADING_OPT_CATEGORY; ?></td>
...
<?php
    ...
    $java_pop = '<script type="text/javascript" language="javascript"><!--' . "\n" .
      'static_options = new Array();' . "\n";
...
<?php
      if (($action == 'update_option') && ($HTTP_GET_VARS['option_id'] == $options_values['products_options_id'])) {
        ...
        for ($i = 0, $n = sizeof($languages); $i < $n; $i ++) {
          ...
          if($options_values['products_options_is_static'] == 1){
            $java_pop .= "static_options[" . (string)($rows-1) . "] = " . chr(34)  . $options_values["products_options_name"]  . chr(34) .";\n";
          }
          ...
          $category_content = '<select name="option_category[' . $languages[$i]['id'] . ']">';
          $top_categories = tep_db_query(
            "select * ".
            "from " . TABLE_CATEGORIES . " c INNER JOIN " . TABLE_CATEGORIES_DESCRIPTION . " cd ".
            "ON cd.categories_id = c.categories_id ".
            "WHERE c.parent_id=0 AND cd.language_id = '" . $languages[$i]['id'] . "'");
          while ($top_categories_values = tep_db_fetch_array($top_categories)) {
            $category_content .= '<option value="' . $top_categories_values['categories_id'] . '"';
            if ($top_categories_values['categories_id'] == $options_values["categories_id"]) {
              $category_content .= ' SELECTED';
            }
            $category_content .= '>' . $top_categories_values['categories_name'] .'</option>';
          }
...
?>
                ...
                <td align="center" class="smallText"><?php echo tep_draw_checkbox_field('option_static','',$options_values['products_options_is_static'] == 1 ? true : false); ?></td>
                <td align="center" class="smallText"><?php echo tep_draw_checkbox_field('option_visible','',$options_values['products_options_is_visible'] == 1 ? true : false); ?></td>
                <td class="smallText"><?php echo $category_content; ?></td>
...
<?php
        echo '</form>' . "\n";
         ...
         $category_content = $options_values['categories_name'];
?>
...
<?php
      ...
      for ($i = 0, $n = sizeof($languages); $i < $n; $i ++) {
        ...
        $top_categories = tep_db_query(
          ...
          "WHERE c.parent_id=0 AND cd.language_id = '" . $languages[$i]['id'] . "'");
        while ($top_categories_values = tep_db_fetch_array($top_categories)) {
          $category_content .= '<option value="' . $top_categories_values['categories_id'] . '">' . $top_categories_values['categories_name'] .'</option>';
        }
        $category_content .= '</select>';
      }
?>
                ...
                <td class="smallText"><?php echo $inputs; ?></td>
                <td align="center" class="smallText"><?php echo tep_draw_checkbox_field('option_static','',false); ?></td>
                <td align="center" class="smallText"><?php echo tep_draw_checkbox_field('option_visible','',true); ?></td>
                <td class="smallText"><?php echo $category_content; ?></td>
...
-->
            <td align="right" class="smallText"><span id="svalue_price" style="visibility:<?php echo $is_static=='1' ? 'hidden' : 'visible' ?>">&nbsp;<input type="text" name="value_price" value="<?php echo $attributes_values['options_values_price']; ?>" size="6">&nbsp;</span></td>
            <td align="center" class="smallText"><span id="sprice_prefix" style="visibility:<?php echo $is_static=='1' ? 'hidden' : 'visible' ?>">&nbsp;<input type="text" name="price_prefix" value="<?php echo $attributes_values['price_prefix']; ?>" size="2">&nbsp;</span></td>
            <td align="center" class="smallText">&nbsp;<?php echo tep_image_submit('button_update.gif', IMAGE_UPDATE); ?>&nbsp;<?php echo '<a href="' . tep_href_link(FILENAME_PRODUCTS_ATTRIBUTES, $page_info, 'NONSSL') . '">'; ?><?php echo tep_image_button('button_cancel.gif', IMAGE_CANCEL); ?></a>&nbsp;</td>
<?php
      if (DOWNLOAD_ENABLED == 'true') {
...
?>
            ...
            <td class="smallText">&nbsp;<select name="options_id" onChange="toggleT(document.attributes.options_id.options[selectedIndex].text)">
<?php
    $options = tep_db_query("select * from " . TABLE_PRODUCTS_OPTIONS . " where language_id = '" . $languages_id . "' order by products_options_name");
    $first = 1;
    while ($options_values = tep_db_fetch_array($options)) {
      if($first){
        $is_static = $options_values['products_options_is_static'];
        $first = 0;
      }
      echo '<option name="' . $options_values['products_options_name'] . '" value="' . $options_values['products_options_id'] . '">' . $options_values['products_options_name'] . '</option>';
    }
...
-->
            <td align="right" class="smallText"><span id="svalue_price" style="visibility:<?php echo $is_static=='1' ? 'hidden' : 'visible' ?>">&nbsp;<input type="text" name="value_price" size="6">&nbsp;</span></td>
            <td align="right" class="smallText"><span id="sprice_prefix" style="visibility:<?php echo $is_static=='1' ? 'hidden' : 'visible' ?>">&nbsp;<input type="text" name="price_prefix" size="2" value="+">&nbsp;</span></td>
            <td align="center" class="smallText">&nbsp;<?php echo tep_image_submit('button_insert.gif', IMAGE_INSERT); ?>&nbsp;</td>
          </tr>
<?php
      if (DOWNLOAD_ENABLED == 'true') {
