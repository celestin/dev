 ...
 * CAM  30-Oct-2010  10800 : Added RRP to results grid.
 * * * * * * * * * * * * * * * * * * * * * * * */
...
function product_price ($listing,$last=false) {
    ...
    if ($product_rrp = tep_db_fetch_array($product_rrp_query)) {
      if (!empty($product_rrp['products_extra_fields_value'])) {
        $rrp = '<span class="productListing-rrp">' . $currencies->display_price($product_rrp['products_extra_fields_value'], 0) . '</span><br />';
      }
    }
