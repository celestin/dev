 ...
 *
 * Who  When         Why
 * CAM  28-Apr-2010  10636 : Add Product Options to Product Edit page (http://addons.oscommerce.com/info/1538)
 * CAM  28-Apr-2010  10639 : Added Product Extra Fields (http://www.oscommerce.com/community/contributions,2202)
 * CAM  28-Apr-2010  10645 : Ensure images upload correctly for New and Update products
 * CAM  03-Jun-2010  10684 : Move Product Attributes to the top of the screen.
 * CAM  05-Jun-2010  10684 : Format Product Attributes and show only those for the current product's "top category".
 * CAM  12-Jun-2010  10677 : Enlarge Product Name and Description boxes.
 * CAM  22-Jun-2010  10714 : Don't show the 'static' options in the attributes edit (they are controlled by script only).
 * * * * * * * * * * * * * * * * * * * * * * * */
...
// removing categories can be a lengthy process
      ...
      case 'update_product':
        ...
        if (isset($HTTP_POST_VARS['edit_x']) || isset($HTTP_POST_VARS['edit_y'])) {
          ...
          // BOF [#10636]
          $products_options_id = (isset($HTTP_POST_VARS['products_options_id']) ? $HTTP_POST_VARS['products_options_id'] : '');
          $insert_products_options = (isset($HTTP_POST_VARS['insert_products_options']) ? $HTTP_POST_VARS['insert_products_options'] : '');
          $delete_products_options = (isset($HTTP_POST_VARS['delete_products_options']) ? $HTTP_POST_VARS['delete_products_options'] : '');

          foreach($products_options_values_id as $option_id => $option_value_id) {

            if (!empty($delete_products_options[$option_id])) {
              tep_db_query("DELETE FROM " . TABLE_PRODUCTS_ATTRIBUTES . " WHERE products_id=" . (int)$products_id . " AND options_id=" . (int)$option_id);
            } else if (!empty($insert_products_options[$option_id])) {
              tep_db_query("INSERT INTO " . TABLE_PRODUCTS_ATTRIBUTES . " (products_id,options_id,options_values_id) ".
              "VALUES (" . (int)$products_id . "," . (int)$option_id . "," . (int)$option_value_id . ")");
            } else {
              tep_db_query("UPDATE " . TABLE_PRODUCTS_ATTRIBUTES . " SET options_values_id=". (int)$option_value_id . " ".
               " WHERE products_id=" . (int)$products_id . " AND options_id=" . (int)$option_id);
            }
          }
          // EOF [#10636]
...
//--></script>
    <?php echo tep_draw_form('new_product', FILENAME_CATEGORIES, 'cPath=' . $cPath . (isset($HTTP_GET_VARS['pID']) ? '&pID=' . $HTTP_GET_VARS['pID'] : '') . '&action=' . (isset($HTTP_GET_VARS['pID']) ? 'update_product' : 'insert_product'), 'post', 'enctype="multipart/form-data"'); ?>
    <?php echo tep_image(DIR_WS_CATALOG_IMAGES . $pInfo->products_image, $pInfo->products_model, $width = '', $height = '', 'id="productimage" hspace="5" vspace="5"') ; ?>
...
?>
            <td class="pageHeading"><?php echo tep_output_generated_category_path($current_category_id) . " > " .
...
?>
      <tr>
        <td><table border="0" width="500" cellspacing="0" cellpadding="2" style="border: 1px solid black;margin-bottom:20px;">
          <tr class="dataTableHeadingRow">
            <th class="dataTableHeadingContent">Option</th>
            <th class="dataTableHeadingContent">Value</th>
            <th class="dataTableHeadingContent">Add</th>
            <th class="dataTableHeadingContent">Delete</th>
          </tr>
<?
      $categories_array = tep_generate_category_path($current_category_id);
      $top_category_id = 0;
      foreach($categories_array[0] as $k => $v) {
        $top_category_id = $v['id'];
      }

      $options = tep_db_query(
        "SELECT * FROM " . TABLE_PRODUCTS_OPTIONS . " " .
        "where language_id = '" . $languages_id . "' ".
        "AND categories_id = '" . $top_category_id . "' ".
        "AND products_options_is_static=1 ".
...
?>
        <tr class="<?php echo (floor($rows/2) == ($rows/2) ? 'attributes-even' : 'attributes-odd'); ?>">
          ...
          <td class="smallText"><select name="products_options_values_id[<? echo $options_values['products_options_id'];?>]" id="products_options_values_id[<? echo $options_values['products_options_id'];?>]">
<?
        $values = tep_db_query(
          "SELECT pov.products_options_values_id, pov.products_options_values_name, pa.options_values_id ".
          "FROM " . TABLE_PRODUCTS_OPTIONS_VALUES . " pov INNER JOIN " .
          TABLE_PRODUCTS_OPTIONS_VALUES_TO_PRODUCTS_OPTIONS . " povp ".
          "ON povp.products_options_id = ". $options_values['products_options_id'] . " ".
          "AND povp.products_options_values_id = pov.products_options_values_id LEFT OUTER JOIN ".
          TABLE_PRODUCTS_ATTRIBUTES . " pa ".
          "ON pa.options_id = povp.products_options_id ".
          "AND pa.options_values_id = povp.products_options_values_id ".
          "AND pa.products_id ='" . (isset($HTTP_GET_VARS['pID']) ? $HTTP_GET_VARS['pID'] : '') . "' ".
        ...
        while($values_values = tep_db_fetch_array($values)) {
          $selected = "";
          if (!empty($values_values['options_values_id'])) {
            $selected = " SELECTED";
            $options_values_id = $values_values['options_values_id'];
          }
?>
          <option name="<? echo $values_values['products_options_values_name']; ?>" <? echo $selected;?> value="<? echo $values_values['products_options_values_id']; ?>"><? echo $values_values['products_options_values_name']; ?></option>
<?
        }
?>
        </select></td>
        <td align="center">
<? if (empty($options_values_id)) { ?>
          <input name="insert_products_options[<? echo $options_values['products_options_id'];?>]" id="insert_products_options[<? echo $options_values['products_options_id'];?>]" type=checkbox value="X">
<?
  } else {
    echo "&nbsp;";
  }
?>
        ...
        <td align="center">
<? if (!empty($options_values_id)) { ?>
          <input name="delete_products_options[<? echo $options_values['products_options_id'];?>]" id="delete_products_options[<? echo $options_values['products_options_id'];?>]" type=checkbox value="X">
<?
  } else {
    echo "&nbsp;";
  }
...
?>
      ...
      <tr>
        <td><table border="0" cellspacing="0" cellpadding="2">
          <tr>
            <td class="lbl"><?php echo TEXT_PRODUCTS_STATUS; ?></td>
          ...
          <tr>
            <td class="lbl"><?php echo TEXT_PRODUCTS_DATE_AVAILABLE; ?><br><small>(YYYY-MM-DD)</small></td>
          ...
          <tr>
            <td class="lbl"><?php echo TEXT_PRODUCTS_MANUFACTURER; ?></td>
...
?>
          <tr>
            <td class="lbl"><?php if ($i == 0) echo TEXT_PRODUCTS_NAME; ?></td>
            <td class="main"><?php echo tep_image(DIR_WS_CATALOG_LANGUAGES . $languages[$i]['directory'] . '/images/' . $languages[$i]['image'], $languages[$i]['name']) . '&nbsp;' . tep_draw_input_field('products_name[' . $languages[$i]['id'] . ']', (isset($products_name[$languages[$i]['id']]) ? stripslashes($products_name[$languages[$i]['id']]) : tep_get_products_name($pInfo->products_id, $languages[$i]['id'])), ' size="90" '); ?></td>
          </tr>
<?php
    }
?>
          ...
          <tr bgcolor="#ebebff">
            <td class="lbl"><?php echo TEXT_PRODUCTS_TAX_CLASS; ?></td>
            <td class="main"><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_pull_down_menu('products_tax_class_id', $tax_class_array, $pInfo->products_tax_class_id, 'onchange="updateGross()"'); ?></td>
          </tr>
          <tr bgcolor="#ebebff">
            <td class="lbl"><?php echo TEXT_PRODUCTS_PRICE_NET; ?></td>
            <td class="main"><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_input_field('products_price', $pInfo->products_price, 'onKeyUp="updateGross()"'); ?></td>
          </tr>
          <tr bgcolor="#ebebff">
            <td class="lbl"><?php echo TEXT_PRODUCTS_PRICE_GROSS; ?></td>
...
?>
          <tr>
            <td class="lbl" valign="top"><?php if ($i == 0) echo TEXT_PRODUCTS_DESCRIPTION; ?></td>
            <td><table border="0" cellspacing="0" cellpadding="0">
              <tr>
                <td class="lbl" valign="top"><?php echo tep_image(DIR_WS_CATALOG_LANGUAGES . $languages[$i]['directory'] . '/images/' . $languages[$i]['image'], $languages[$i]['name']); ?>&nbsp;</td>
                <td class="main"><?php echo tep_draw_textarea_field('products_description[' . $languages[$i]['id'] . ']', 'soft', '87', '4', (isset($products_description[$languages[$i]['id']]) ? stripslashes($products_description[$languages[$i]['id']]) : tep_get_products_description($pInfo->products_id, $languages[$i]['id']))); ?></td>
              </tr>
            </table></td>
          </tr>
<?php
...
?>
          ...
          <tr>
            <td class="lbl"><?php echo TEXT_PRODUCTS_QUANTITY; ?></td>
          ...
          <tr>
            <td class="lbl"><?php echo TEXT_PRODUCTS_MODEL; ?></td>
          ...
          <tr>
            <td class="lbl"><?php echo TEXT_PRODUCTS_IMAGE; ?></td>
...
?>
          <tr>
            <td class="lbl"><?php if ($i == 0) echo TEXT_PRODUCTS_URL . '<br><small>' . TEXT_PRODUCTS_URL_WITHOUT_HTTP . '</small>'; ?></td>
...
?>
          ...
          <tr>
            <td class="lbl"><?php echo TEXT_PRODUCTS_WEIGHT; ?></td>
            <td class="main"><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_input_field('products_weight', $pInfo->products_weight); ?></td>
          </tr>
<?php
// BOF Extra Fields Contribution (chapter 1.4)  [#10639]
      // Sort language by ID
    for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
...
?>
          <tr bgcolor="#ebebff">
            <td class="lbl"><?php echo $extra_fields['products_extra_fields_name']; ?></td>
...
?>
      ...
      <tr>
        ...
        (isset($HTTP_GET_VARS['pID']) ? tep_image_submit('button_update.gif', IMAGE_UPDATE) : tep_image_submit('button_insert.gif', IMAGE_INSERT)).
        '&nbsp;&nbsp;<a href="' . tep_href_link(FILENAME_CATEGORIES, 'cPath=' . $cPath . (isset($HTTP_GET_VARS['pID']) ? '&pID=' . $HTTP_GET_VARS['pID'] : '')) . '">' .
...
?>
    <table border="0" width="100%" cellspacing="0" cellpadding="2">
      ...
      <tr>
        <td class="main">
         <?php
          echo tep_image(DIR_WS_CATALOG_IMAGES . $products_image_name, $pInfo->products_name, $width = '', $height = '', 'align="right" hspace="5" vspace="5"');
 // BOF Extra Fields Contribution [#10639]
        ...
        <td><table border="0" width="100%" cellspacing="0" cellpadding="0">
          <tr>
            <td valign="top"><table border="0" width="100%" cellspacing="0" cellpadding="2">
              <tr class="dataTableHeadingRow">
                <!-- BOF Extra Fields Contribution / RELENAT FIELDS [#10639] -->
                <td class="dataTableHeadingContent">ID</td>
                <!-- EOF Extra Fields Contribution / RELENAT FIELDS [#10639] -->
      ...
      default:
        if ($rows > 0) {
          if (isset($cInfo) && is_object($cInfo)) { // category info box contents
            ...
            $contents[] = array('text' => '<br>' . tep_info_image($cInfo->categories_image, $cInfo->categories_name, 350, 0) . '<br>' . $cInfo->categories_image);
            ...
            $contents[] = array('text' => '<br>' . tep_info_image($pInfo->products_image, $pInfo->products_name, 0, SMALL_IMAGE_HEIGHT) . '<br>' . $pInfo->products_image);
