...
// the following cPath references come from application_top.php
  ...
  function display_categories($categories, $title, $text) {
    global $languages_id;

    $small_categories_query = tep_db_query(
      "SELECT c.categories_id, cd.categories_name, c.categories_image ".
      "FROM " . TABLE_CATEGORIES . " c, " .  TABLE_CATEGORIES . " cp, " . TABLE_CATEGORIES_DESCRIPTION . " cd ".
      "WHERE c.parent_id IN (" . $categories . ") ".
      "AND c.categories_id = cd.categories_id ".
      "AND cp.categories_id = c.parent_id ".
      "AND cd.language_id = '" . (int)$languages_id . "' ".
      "ORDER BY cp.sort_order, c.sort_order, cd.categories_name");
...
// check to see if there are deeper categories within the current category
    ...
    while ($categories = tep_db_fetch_array($categories_query)) {
      ...
      echo '                <td align="center" class="smallText" width="' . $width . '" valign="top"><a href="' . tep_href_link(FILENAME_DEFAULT, $cPath_new) . '">' . tep_image(DIR_WS_IMAGES . $categories['categories_image'], $categories['categories_name'], 0, SUBCATEGORY_IMAGE_HEIGHT) . $categories_name . '</a></td>' . "\n";
