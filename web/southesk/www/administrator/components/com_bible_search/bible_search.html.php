<?php
/**
* @version $Id: bible_search.html.php,v 1.35 2004/09/26 10:37:23 stingrey Exp $
* @package Mambo_4.5.1
...
* @package Mambo_4.5.1
*/
class HTML_bible_search {

	function viewbible_search( &$bible_search, &$params, $count, &$list ) {
		//print_r( $bible_search );
		...
		function validate(){
			...
			document.emailForm.action = "<?php echo sefRelToAbs("index.php?option=com_bible_search&Itemid=$Itemid"); ?>"
		...
		function ViewCrossReference( selSelectObject ){
			...
			for ($i = 0; $i < $n; $i++) {
			   echo "\nlinks[".$list[$i]->value."]='"
					. sefRelToAbs( 'index.php?option=com_bible_search&task=view&bible_search_id='. $list[$i]->value .'&Itemid='. $Itemid )
					. "';";
			}
			?>
		...
		if ( $params->get( 'popup' ) ) {
			?>
			<title><?php echo $sitename ." :: ". $bible_search->name; ?></title>
		...
		<?php
		// displays Page Title
		HTML_bible_search::_writePageTitle( $params );
	}
	...
	* Writes Image
	*/
	function _writeImage( &$bible_search, &$params ) {
		global $mosConfig_live_site;
		if ( $bible_search->image && $params->get( 'image' ) ) {
			?>
			<div style="float: right;">
			<img src="<?php echo $mosConfig_live_site;?>/images/stories/<?php echo $bible_search->image; ?>" align="middle" alt="Contact" />
			</div>
			<?php
		}
	}
	...
	* Writes Address
	*/
	function _writeContactAddress( &$bible_search, &$params ) {
		if ( ( $params->get( 'address_check' ) > 0 ) &&  ( $bible_search->address || $bible_search->suburb  || $bible_search->state || $bible_search->country || $bible_search->postcode ) ) {
			global $mosConfig_live_site;
			?>
			...
			?>
			<?php
			if ( $bible_search->address && $params->get( 'street_address' ) ) {
				?>
				<tr>
					<td valign="top">
					<?php
					echo $bible_search->address;
					?>
					</td>
				</tr>
				<?php
			}
			if ( $bible_search->suburb && $params->get( 'suburb' ) ) {
				?>
				<tr>
					<td valign="top">
					<?php
					echo $bible_search->suburb;
					?>
					</td>
				</tr>
				<?php
			}
			if ( $bible_search->state && $params->get( 'state' ) ) {
				?>
				<tr>
					<td valign="top">
					<?php
					echo $bible_search->state;
					?>
					</td>
				</tr>
				<?php
			}
			if ( $bible_search->country && $params->get( 'country' ) ) {
				?>
				<tr>
					<td valign="top">
					<?php
					echo $bible_search->country;
					?>
					</td>
				</tr>
				<?php
			}
			if ( $bible_search->postcode && $params->get( 'postcode' ) ) {
				?>
				<tr>
					<td valign="top">
					<?php
					echo $bible_search->postcode;
					?>
					</td>
				</tr>
				<?php
	...
	* Writes Contact Info
	*/
	function _writeContactContact( &$bible_search, &$params ) {
		if ( $bible_search->email_to || $bible_search->telephone  || $bible_search->fax ) {
			global $mosConfig_live_site;
			?>
			<table width="100%" cellpadding="0" cellspacing="0" border="0">
			<?php
			if ( $bible_search->email_to && $params->get( 'email' ) ) {
				?>
				<tr>
					...
					<td>
					<?php
					echo $bible_search->email;
					?>
					</td>
				</tr>
				<?php
			}
			if ( $bible_search->telephone && $params->get( 'telephone' ) ) {
				?>
				<tr>
					<td width="<?php echo $params->get( 'column_width' ); ?>" align="left">
					<?php
					...
					<td>
					<?php
					echo $bible_search->telephone;
					?>
					</td>
				</tr>
				<?php
			}
			if ( $bible_search->fax && $params->get( 'fax' ) ) {
				?>
				<tr>
					<td width="<?php echo $params->get( 'column_width' ); ?>" align="left">
					<?php
	...
	* Writes Email form
	*/
	function _writeEmailForm( &$bible_search, &$params, $sitename ) {
		if ( $bible_search->email_to && !$params->get( 'popup' ) && $params->get( 'email_form' ) ) {
			?>
...
<TABLE border=0 cellPadding=8 cellSpacing=0 width=650>
  ...
  <TR>
	...
	function nobible_search( &$params ) {
		?>
		<br /><br />
