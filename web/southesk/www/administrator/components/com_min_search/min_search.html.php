<?php
/**
* @version $Id: min_search.html.php,v 1.35 2004/09/26 10:37:23 stingrey Exp $
* @package Mambo_4.5.1
...
* @package Mambo_4.5.1
*/
class HTML_min_search {

  function viewmin_search( &$min_search, &$params, $count, &$list ) {
    //print_r( $min_search );
    ...
    function validate(){
      ...
      document.emailForm.action = "<?php echo sefRelToAbs("index.php?option=com_min_search&Itemid=$Itemid"); ?>"
    ...
    function ViewCrossReference( selSelectObject ){
      ...
      for ($i = 0; $i < $n; $i++) {
         echo "\nlinks[".$list[$i]->value."]='"
          . sefRelToAbs( 'index.php?option=com_min_search&task=view&min_search_id='. $list[$i]->value .'&Itemid='. $Itemid )
          . "';";
      }
      ?>
    ...
    if ( $params->get( 'popup' ) ) {
      ?>
      <title><?php echo $sitename ." :: ". $min_search->name; ?></title>
    ...
    <?php
    // displays Page Title
    HTML_min_search::_writePageTitle( $params );
  }
  ...
  * Writes Email form
  */
  function _writeEmailForm( &$min_search, &$params, $sitename ) {
    if ( $min_search->email_to && !$params->get( 'popup' ) && $params->get( 'email_form' ) ) {
      ?>
